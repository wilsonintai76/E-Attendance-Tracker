...
export default function PolikuMapInner({
  ...
  const updateLocation = (lat: number, lng: number) => {
    ...
    if (onChange) onChange(parseFloat(lat.toFixed(6)), parseFloat(lng.toFixed(6)));
  };
  ...
  useEffect(() => {
    ...
    if (studentLocation) {
      ...
      if (studentMarkerRef.current) {
        ...
        studentMarkerRef.current = L.marker([sLat, sLng], { icon: studentIcon }).addTo(mapRef.current);
      }

      const bounds = L.latLngBounds([[latitude, longitude], [sLat, sLng]]);
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (onChange) onChange(parseFloat(currentLat.toFixed(6)), parseFloat(currentLng.toFixed(6)));
          if (mapRef.current) mapRef.current.setView([currentLat, currentLng], 17);
        },
        (error) => {
          toast.dismiss('gps-locate');
          console.error(error);
          toast.error('Could not access GPS. Please ensure browser location permissions are granted.');
        },
  ...
  const handleMapSearch = async (e?: React.FormEvent | React.MouseEvent | React.KeyboardEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const found = data[0];
        updateLocation(parseFloat(found.lat), parseFloat(found.lon));
        ...
        if (fallbackData && fallbackData.length > 0) {
          ...
          toast.error('Location not found. Try searching for specific campus block names.');
        }
      }
    } catch (err) {
      console.error(err);
