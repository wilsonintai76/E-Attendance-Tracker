...
export default function LecturerDashboard() {
  ...
  const [editSessionWeek, setEditSessionWeek] = useState('1');
  const [editSessionStatus, setEditSessionStatus] = useState<'active' | 'inactive'>('inactive');
  ...
  const handleEditSession = (sess: AttendanceSession) => {
    ...
    setEditSessionWeek(sess.week?.toString() || '1');
    setEditSessionStatus(sess.status);
    setShowEditSessionModal(true);
  };

  const handleSaveSession = async () => {
    if (!editingSession) return;
    const week = parseInt(editSessionWeek) || undefined;
    const status = editSessionStatus;

    const updated = sessions.map(s => s.id === editingSession.id ? { ...s, date: editSessionDate, week, status } : s);
    setSessions(updated);
    setShowEditSessionModal(false);
    api.updateSession(editingSession.id, { date: editSessionDate, week, status }).then(() => refreshData()).catch(() => {});
